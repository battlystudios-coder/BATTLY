...
class NewsPanel {
  ...
  async First() {
    ...
    document.getElementById('XXXXXXXX').addEventListener("mouseenter", () => {
      const audio = new Audio("https://www.myinstants.com/media/sounds/gta-san-andreas-ah-shit-here-we-go-again.mp3")
      audio.play();
    });
    ...
    function terminar() {
      ...
      let textoImportante = `Non toccare i file nelle cartelle interne di Battly. Si tratta di file necessari a Battly per funzionare. Se tocchi qualcosa e il gioco non funziona, NON saremo ritenuti responsabili. Team di Battly.`
      ...
      setTimeout(async () => {
        localStorage.setItem('news_shown_v2.0', true);

        if (thiss.accounts.length == 0) {
          new Alert().ShowAlert({
            title: 'Benvenuto/a!',
            text: 'Per continuare, effettua l\'accesso o crea un account.',
            icon: 'info',
          });
          changePanel('login');
        } else {
