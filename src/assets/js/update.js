...
class Splash {
    ...
    async load() {
        try {
            const res = await fetch("https://api.battly.site/config.json");
    ...
    async downloadFileWithProgress(url, dest) {
        ...
        if (!response.ok) throw new Error(`Impossibile scaricare ${url}: ${response.statusText}`);
        ...
        progressStream.on('progress', (progress) => {
            ...
            this.log(`Download in corso... ${percentage}%`);
    ...
    async unzipWithProgress(zipPath, outputPath) {
        ...
        for (const entry of zipEntries) {
            ...
            this.log(`Decompressione... ${progress}%`);
            this.updateProgress(progress);

            if (extractedFiles === totalFiles) {
                await fs.unlinkSync(zipPath);
                this.log('Aggiornamento completato! Riavvio in corso...');
                setTimeout(() => {
    ...
    async downloadAndUnzip(url, zipPath, outputPath) {
        try {
            ...
            console.error('Errore durante il download o la decompressione:', error);
            this.log('Errore durante il download o la decompressione del file di aggiornamento');
            this.log(error);
        }
    }
}
...
document.addEventListener("keydown", (e) => {
    if (e.ctrlKey && e.shiftKey && (e.keyCode === 73 || e.keyCode === 123)) {
        ...
        console.log("%cASPETTA!", "color: #d03e3eff; font-size: 70px; font-weight: bold; font-family: 'Poppins'; text-shadow: 0 0 5px #000;");
        console.log("%cNon fare nulla qui se non sai cosa stai facendo!", "color: #d03e3eff; font-size: 18px; font-weight: bold; font-family: 'Poppins';");
        console.log("%cNon infilare niente di esterno qui dentro: c'è il 101% di probabilità che si tratti di un virus!", "color: #d03e3eff; font-size: 15px; font-weight: bold; font-family: 'Poppins';");
    }
});
