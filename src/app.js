...
const socket = io("https://api.battly.site");
...
app.whenReady().then(() => {
  ...
  const contextMenu = Menu.buildFromTemplate([
    ...
    },
    {
      ...
      type: "normal",
      click: () => {
        shell.openExternal("https://battly.site");
      },
    },
    { type: "separator" },
    {
...
ipcMain.on("new-status-discord", async () => {
  client.login({ clientId: "1420139811062153246" });
  client.on("ready", () => {
    client
      .request("SET_ACTIVITY", {
        ...
        activity: {
          ...
          buttons: [
            {
              label: "👥 Discord",
              url: "https://discord.battly.site",
            },
            { label: "⏬ Scarica", url: "https://battly.site" },
          ],
          instance: false,
          timestamps: {
...
ipcMain.handle("update-new-app", async () => {
  ...
  return new Promise(async (resolve, reject) => {
    fetch("https://api.battly.site/config.json").then(async res => {
